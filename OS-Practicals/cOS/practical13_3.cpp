...
int main()
{
    ...
    cout << endl;
    cout << "PID Process Size Block Number Block Size Internal Fragmentation" << endl;
    for (int i = 0; i < N_process; i++)
    {
        for (int j = 0; j < N_memory; j++)
        {
            if (memory[j].allocated)
                continue;
            else if (process[i].process_size <= memory[j].block_size)
            {
                process[i].block_number = memory[j].block_number;
                memory[j].PID = process[i].PID;
                memory[j].allocated = true;
                process[i].assigned = true;
                memory[j].internal_fragmentation = memory[j].block_size - process[i].process_size;
